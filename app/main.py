...
def main():
        ...
        if uploaded_file is not None:
            try:
                ...
                for category_outline in root.findall("./body/outline"):
                    category = category_outline.get("text") or category_outline.get(
                        "title"
                    )
                    ...
                    for feed_outline in category_outline.findall("outline"):
                        ...
                        if feed_url:
                            title = feed_outline.get("text") or feed_outline.get(
                                "title"
                            )
                            ...
                            imported_count += 1
                st.success(
                    f"Successfully imported {imported_count} feeds from OPML file!"
                )
    ...
    with overview_tab:
        ...
        for category, category_feeds in feeds_by_category.items():
            with st.expander(
                f"📁 {category} ({len(category_feeds)} feeds)", expanded=True
            ):
                    ...
                    with col1:
                        if st.session_state[edit_key]:
                            new_title = st.text_input(
                                "Title", feed.title or feed.url, key=f"title_{feed.id}"
                            )
                            new_category = st.text_input(
                                "Category",
                                feed.category or "",
                                key=f"category_{feed.id}",
                            )
                            if st.button("Save", key=f"save_{feed.id}"):
                                feed_service.update_feed(
                                    feed.id, new_category, new_title
                                )
                                st.session_state[edit_key] = False
                                st.rerun()
                        else:
            ...
            for entry in entries:
                with st.container():
                    ...
                    with col1:
                        ...
                        st.markdown(
                            f"*{category} / {entry.feed.title}* - {entry.published_date.strftime('%Y-%m-%d %H:%M')}"
                        )
                        st.markdown("---")
    ...
    with feeds_tab:
        ...
        if not feeds:
            ...
            selected_feed = st.selectbox(
                "Select Feed", options=feeds, format_func=lambda x: x.title or x.url
            )
                ...
                for entry in entries:
                    with st.expander(entry.title):
                        ...
                        if entry.llm_summary:
                            ...
                            if st.button(
                                "🤖 Generate LLM Summary", key=f"llm_{entry.id}"
                            ):
                                with st.spinner("Generating summary..."):
                                    feed_service.generate_llm_summary(entry.id)
                                st.rerun()
