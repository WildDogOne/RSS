    ...
    if st.session_state.settings_open:
        ...
        with st.container():
            ...
            if (
                ...
                or st.session_state.ollama_url != st.session_state.get("new_url")
            ):
            ...
            if st.button("Add Feed"):
                if new_feed_url:
                    try:
                        ...
                        st.success(
                            f"Feed added successfully{' and entries loaded' if feed else ''}"
                        )
                        st.rerun()
                    except Exception as e:
    ...
    if st.session_state.console_open:
        ...
        with st.sidebar:
            ...
            if st.session_state.debug_enabled:
                ...
                st.warning(
                    "Debug logging is disabled. Enable it in Settings to view logs."
                )
...
def render_ioc_view(feed_service: FeedService):
    ...
    for ioc in filtered_iocs:
        with st.expander(f"{ioc['type']}: {ioc['value']}", expanded=True):
            ...
            st.progress(
                ioc["confidence_score"] / 100,
                text=f"Confidence: {ioc['confidence_score']}%",
            )
        ...
        if st.session_state.scheduler is None:
            ...
            scheduler.add_job(
                lambda: FeedService(
                    SessionFactory(),
                    LLMService(
                        st.session_state.ollama_url, st.session_state.ollama_model
                    ),
                ).update_feeds(),
                "interval",
                minutes=30,
